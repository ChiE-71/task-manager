...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    if (existingUser) {
      ...
      await pool.query(
        "INSERT INTO tasks (user_id, task, created_at) VALUES ($1, $2, CURRENT_TIMESTAMP)",
        [
          newUserId,
          "Welcome! Here's your first task! Create new task for yourself!",
        ],
      );
      ...
      await pool.query("UPDATE users SET failed_attempts = 0 WHERE id = $1", [
        newUser.rows[0].id,
      ]);
